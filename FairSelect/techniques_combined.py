...
def run_combined_pipeline(model_name, params,
    ...
    if "Post:Reject-Option Shift" in post_plan and P_val is not None:
        ...
        roc_model, roc_metadata = fit_aif360_reject_option(
            y_val=yva,
            p_val=P_val,
            A_val=Ava,
        ...
        test_dataset = _make_aif360_prediction_dataset(
            Ate,
            p_test,
            group_mapping,
            labels=(np.asarray(p_test) >= 0.5).astype(int),
        )
    ...
    combined_predictor = CombinedPipelinePredictor(
        ...
        reject_option_model=aif360_roc_for_fairmodel,
        reject_option_group_mapping=aif360_roc_group_mapping,
