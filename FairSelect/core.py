...
class FairSelectPlanSpec:
    ...
    selected: Tuple[Tuple[str, bool],...] = field(default_factory=tuple)
    train_index: Tuple[Any,...] = field(default_factory=tuple)
    validation_index: Tuple[Any,...] = field(default_factory=tuple)
    test_index: Tuple[Any,...] = field(default_factory=tuple)
    random_state: int = 42
...
def build_estimator(model_name, params=None):
    ...
    model_aliases = {"logistic_regression": "logistic_regression", "logistic": "logistic_regression", "logreg": "logistic_regression", "lr": "logistic_regression",
                    "neural_network": "neural_network", "neural": "neural_network", "nn": "neural_network", "mlp": "neural_network", "mlpclassifier": "neural_network",
                    ...
                    "svm": "svm", "svc": "svm", "support_vector_machine": "svm", "support_vector_classifier": "svm",
                    "xgboost": "xgboost", "xgb": "xgboost", "xgbclassifier": "xgboost",
                    "lightgbm": "lightgbm", "light_gbm": "lightgbm", "lgbm": "lightgbm", "lgbmclassifier": "lightgbm",}
    
    canonical_name = model_aliases.get(normalized_name)
    if canonical_name is None:
        supported_aliases = ", ".join(sorted(model_aliases))
        raise ValueError(f"Unknown model name: {model_name!r}. " f"Supported names and aliases include: {supported_aliases}")
    ...
    if canonical_name == "logistic_regression":
        ...
        if l1_ratio is None:
            l1_ratio = 0.0
        return LogisticRegression(C=float(C_value), l1_ratio=float(l1_ratio), solver=params.get("solver", "lbfgs"), max_iter=int(params.get("max_iter", 200) or 200), 
    ...
    if canonical_name == "random_forest":
        ...
        if (isinstance(class_weight, str) and class_weight.lower() == "none"):
            class_weight = None
        return RandomForestClassifier(n_estimators=int(params.get("n_estimators", 200) or 200), max_depth=params.get("max_depth"),
                                    min_samples_split=int(params.get("min_samples_split", 2) or 2), min_samples_leaf=int(params.get("min_samples_leaf", 1) or 1),
                                    max_features=max_features, class_weight=class_weight, random_state=params.get("random_state"), n_jobs=params.get("n_jobs", -1),)
    
    if canonical_name == "decision_tree":
        max_features = params.get("max_features")
        if (isinstance(max_features, str) and max_features.lower() == "none"):
            max_features = None
        return DecisionTreeClassifier(criterion=params.get("criterion", "gini"), max_depth=params.get("max_depth"),
                                    min_samples_split=int(params.get("min_samples_split", 2) or 2), min_samples_leaf=int(params.get("min_samples_leaf", 1) or 1),
                                    max_features=max_features, class_weight=(None if str(params.get("class_weight", "none")).lower() == "none" else params.get("class_weight")),
    ...
    if canonical_name == "svm":
        ...
        if isinstance(probability, str):
            probability = probability.strip().lower() in {"true", "1", "yes", "y",}
        return SVC(kernel=params.get("kernel", "rbf"), C=float(C_value), gamma=params.get("gamma", "scale"), degree=int(degree), probability=bool(probability),
                class_weight=class_weight, random_state=params.get("random_state"),)
    
    if canonical_name == "xgboost":
        if not XGB_OK:
            raise ImportError("XGBoost was requested, but xgboost is not installed " "or could not be imported.")
        return XGBClassifier(n_estimators=int(params.get("n_estimators", 300) or 300), max_depth=int(params.get("max_depth", 6) if params.get("max_depth", 6) is not None else 6),
                            learning_rate=float(params.get("learning_rate", 0.1) if params.get("learning_rate", 0.1) is not None else 0.1), 
                            subsample=float(params.get("subsample", 1.0) if params.get("subsample", 1.0) is not None else 1.0), 
                            colsample_bytree=float(params.get("colsample_bytree", 1.0) if params.get("colsample_bytree", 1.0) is not None else 1.0), 
                            reg_alpha=float(params.get("reg_alpha", 0.0) if params.get("reg_alpha", 0.0) is not None else 0.0), 
                            reg_lambda=float(params.get("reg_lambda", 1.0) if params.get("reg_lambda", 1.0) is not None else 1.0), random_state=params.get("random_state"),
                            n_jobs=params.get("n_jobs", -1), eval_metric=params.get("eval_metric", "logloss",),)
    
    if canonical_name == "lightgbm":
        if not LGBM_OK:
            raise ImportError("LightGBM was requested, but lightgbm is not installed " "or could not be imported.")
        return LGBMClassifier(n_estimators=int(params.get("n_estimators", 300) or 300),
                            max_depth=int(params.get("max_depth", -1) if params.get("max_depth", -1) is not None else -1), 
                            learning_rate=float(params.get("learning_rate", 0.05) if params.get("learning_rate", 0.05) is not None else 0.05), 
                            num_leaves=int(params.get("num_leaves", 31) if params.get("num_leaves", 31) is not None else 31), 
                            subsample=float(params.get("subsample", 1.0) if params.get("subsample", 1.0) is not None else 1.0), 
                            colsample_bytree=float(params.get("colsample_bytree", 1.0) if params.get("colsample_bytree", 1.0) is not None else 1.0), 
                            reg_alpha=float(params.get("reg_alpha", 0.0) if params.get("reg_alpha", 0.0) is not None else 0.0), 
                            reg_lambda=float(params.get("reg_lambda", 0.0) if params.get("reg_lambda", 0.0) is not None else 0.0), 
                            class_weight=(None if str(params.get("class_weight", "none")).lower() == "none" else params.get("class_weight")), 
                            random_state=params.get("random_state"), n_jobs=params.get("n_jobs", -1), 
                            verbosity=int(params.get("verbosity", -1) if params.get("verbosity", -1) is not None else -1),)
    

    # This should be unreachable because aliases are validated above.
    raise RuntimeError(f"No estimator builder exists for {canonical_name!r}.")
...
def split_data(df: pd.DataFrame, target: str, protected: List[str], features: List[str], test_size=0.25, val_size=0.2, random_state=42):
    """
    ...
    #Extract target column and force to integer
    y = df[target].astype(int)
    X = df[features].copy() #Extract feature columns
    #Split into train / test datasets
    X_tr, X_te, y_tr, y_te, A_tr, A_te = train_test_split(X, y, A, test_size=test_size, stratify=y, random_state=random_state)
    #Split into train / validation datasets
    X_tr, X_va, y_tr, y_va, A_tr, A_va = train_test_split(X_tr, y_tr, A_tr, test_size=val_size, stratify=y_tr, random_state=random_state)
    return X_tr, X_va, X_te, y_tr, y_va, y_te, A_tr, A_va, A_te



def evaluate_run(name: str, y_true, p, yhat, groups, *, fair_model=None, notes: str = "", run_fairlogue: bool = True, test_index=None,) -> RunResult:
    """
    ...
    y_test_saved = (y_true.copy() if hasattr(y_true, "copy") else np.asarray(y_true).copy())
    A_test_saved = (groups.copy() if hasattr(groups, "copy") else np.asarray(groups).copy())
    ...
    if run_fairlogue:
        ...
        fairlogue_results = run_fairlogue_observational(y_true=y_true, y_prob=p, y_pred=yhat, groups=groups, run_name=name,)
    return RunResult(name=name, overall=overall, group_stats=group_df, notes=notes, fairlogue=fairlogue_results, fair_model=fair_model, test_index=test_index_saved, y_test=y_test_saved, A_test=A_test_saved, y_prob_test=np.asarray(p).copy(), y_pred_test=np.asarray(yhat).copy(),)