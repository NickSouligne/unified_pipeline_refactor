...
def _make_aif360_prediction_dataset(groups, scores, group_mapping, labels=None, allow_unknown=False,):
    ...
    labels = ((scores >= 0.5).astype(int) if labels is None else np.asarray(labels, dtype=int).ravel())
    ...
    dataset = BinaryLabelDataset(favorable_label=1.0, unfavorable_label=0.0, df=frame, label_names=[AIF360_LABEL_COL], protected_attribute_names=[AIF360_GROUP_COL],)
...
def run_group_youden_postproc(model_name, params, X_tr, X_va, X_te, y_tr, y_va, y_te, A_tr, A_va, A_te, protected_cols, all_df_train, outcome_col):
    ...
    predictor = StandardPredictor(features=X_tr.columns, protected_cols=protected_cols, preprocessor=prep, estimator=clf, threshold=0.5, group_thresholds=th,)
    fair_model = make_predictor_fair_model(name="Post: Youden per group", features=X_tr.columns, protected_cols=protected_cols, predictor=predictor, 
                                           threshold=0.5, outcome_col=outcome_col, metadata={"source": "FairSelect", "technique": "Post:Youden per group", 
                                                                                             "model_name": model_name, "model_params": params, "group_thresholds": th,},)
    return evaluate_run("Post: Youden per group", y_te.to_numpy(), p_test, yhat, A_te, fair_model=fair_model, test_index=X_te.index,)


def run_multiaccuracy_boost(model_name, params, X_tr, X_va, X_te, y_tr, y_va, y_te, A_tr, A_va, A_te, protected_cols,
                             all_df_train=None, *, alpha=0.02, eta=None, max_iters=25, auditor_type="ridge", random_state=0, eps=1e-6, 
                             include_group_in_auditor=True, outcome_col=None,):
    """
    ...
    # -------------------------------------------------------
    p_adj = apply_multiaccuracy_boost(X_va=X_va, X_te=X_te, y_va=y_va, A_va=A_va, A_te=A_te, p_val=p_val, p_test=p_test, prep=prep, 
                                      alpha=0.02, eta=eta, max_iters=25, auditor_type="ridge", random_state=params.get("random_state", 0), 
                                      include_group_in_auditor=True,)
    yhat = (p_adj >= 0.5).astype(int)
    # -------------------------------------------------------
    # 4. Build FairModel-compatible predictor
    # -------------------------------------------------------
    ma_predictor = MultiaccuracyBoostPredictor(features=list(X_tr.columns), protected_cols=list(protected_cols), preprocessor=prep, estimator=clf, 
                                               X_val=X_va, y_val=y_va, A_val=A_va, p_val=p_val, threshold=0.5, alpha=0.02, eta=eta, max_iters=25, 
                                               auditor_type="ridge", random_state=params.get("random_state", 0), include_group_in_auditor=True,)
    
    fair_model = FairModel(name="Post: Multiaccuracy Boost", features=list(X_tr.columns), protected_cols=list(protected_cols), predictor=ma_predictor,
                            threshold=0.5, outcome_col=outcome_col, positive_label=1, metadata={"source": "FairSelect", "technique": "Post:Multiaccuracy Boost", 
                                                                                                "model_name": model_name, "model_params": params, 
                                                                                                "postprocessing": "multiaccuracy_boost", "alpha": 0.02, "eta": eta, 
                                                                                                "max_iters": 25, "auditor_type": "ridge", "include_group_in_auditor": True,},)
    return evaluate_run("Post: Multiaccuracy Boost", y_te.to_numpy(), p_adj, yhat, A_te, fair_model=fair_model, test_index=X_te.index,)


def fit_aif360_reject_option(y_val, p_val, A_val, group_mapping, unprivileged_values=None, privileged_values=None, metric_name="Average odds difference",
                              metric_lb=-0.05, metric_ub=0.05, low_class_thresh=0.01, high_class_thresh=0.99, num_class_thresh=20, num_ROC_margin=10,):
    ...
    if len(valid_tprs) < 2:
        raise ValueError("Reject Option Classification requires at least two groups " "with defined validation TPR values.")
    unprivileged = ({min(valid_tprs, key=valid_tprs.get)} if unprivileged_values is None else {str(value) for value in unprivileged_values})
    privileged = ({max(valid_tprs, key=valid_tprs.get)} if privileged_values is None else {str(value) for value in privileged_values})
    if unprivileged & privileged:
        raise ValueError("Privileged and unprivileged groups overlap.")
    unprivileged_groups = [{AIF360_GROUP_COL: group_mapping[group]} for group in sorted(unprivileged)]
    privileged_groups = [{AIF360_GROUP_COL: group_mapping[group]} for group in sorted(privileged)]
    true_dataset = _make_aif360_prediction_dataset(A_val, p_val, group_mapping, labels=y_val)
    pred_dataset = _make_aif360_prediction_dataset(A_val, p_val, group_mapping, labels=yhat_val)
    roc_model = RejectOptionClassification(unprivileged_groups=unprivileged_groups, privileged_groups=privileged_groups, low_class_thresh=low_class_thresh, 
                                           high_class_thresh=high_class_thresh, num_class_thresh=num_class_thresh, num_ROC_margin=num_ROC_margin, metric_name=metric_name, 
                                           metric_lb=metric_lb, metric_ub=metric_ub,)
    roc_model.fit(true_dataset, pred_dataset)
    metadata = {"implementation": "AIF360", "metric_name": metric_name, "metric_lb": float(metric_lb), "metric_ub": float(metric_ub),
                 "classification_threshold": float(roc_model.classification_threshold), "ROC_margin": float(roc_model.ROC_margin), 
                 "unprivileged_values": sorted(unprivileged), "privileged_values": sorted(privileged), "validation_tprs": tprs,}
...
def run_reject_option_shift(model_name, params, X_tr, X_va, X_te, y_tr, y_va, y_te, A_tr, A_va, A_te, protected_cols, all_df_train, outcome_col=None, 
    ...
    del all_df_train
    if not AIF360_OK:
        raise ImportError('AIF360 is unavailable. Install it with: pip install "aif360==0.6.1"')
    ...
    for split_name, X, y, A in [("train", X_tr, y_tr, A_tr), ("validation", X_va, y_va, A_va), ("test", X_te, y_te, A_te),]:
        if not (len(X) == len(y) == len(A)):
            raise ValueError(f"{split_name} data are not aligned: X={len(X)}, " f"y={len(y)}, A={len(A)}.")
    ...
    for group in A_val.unique():
        mask = A_val.to_numpy() == group
        validation_tprs[str(group)] = confusion_rates(y_val[mask], yhat_val[mask])["TPR"]
    valid_tprs = {group: value for group, value in validation_tprs.items() if np.isfinite(value)}
    if len(valid_tprs) < 2:
        raise ValueError("Reject Option Classification requires at least two validation " "groups with defined TPR values.")
    ...
    if overlap:
        raise ValueError(f"Groups cannot be both privileged and unprivileged: {sorted(overlap)}")
    ...
    unprivileged_groups = [{AIF360_GROUP_COL: group_mapping[group]} for group in sorted(unprivileged_values)]
    privileged_groups = [{AIF360_GROUP_COL: group_mapping[group]} for group in sorted(privileged_values)]
    validation_true = _make_aif360_prediction_dataset(A_va, p_val, group_mapping, labels=y_val)
    validation_pred = _make_aif360_prediction_dataset(A_va, p_val, group_mapping, labels=yhat_val)
    ...
    test_pred = _make_aif360_prediction_dataset(A_te, p_test, group_mapping, labels=(p_test >= 0.5).astype(int))
    yhat_test = np.asarray(roc_model.predict(test_pred).labels, dtype=int).ravel()

    predictor = AIF360RejectOptionPredictor(features=X_tr.columns, protected_cols=protected_cols, preprocessor=prep, estimator=clf, roc_model=roc_model, 
                                            group_mapping=group_mapping,)
    
    
    metadata = {"source": "FairSelect", "technique": "Post:Reject-Option Shift", "implementation": "AIF360", "postprocessing": "RejectOptionClassification",
                "model_name": model_name, "model_params": dict(params or {}), "metric_name": metric_name, "metric_lb": float(metric_lb),
                "metric_ub": float(metric_ub), "classification_threshold": float(roc_model.classification_threshold), "ROC_margin": float(roc_model.ROC_margin),
                "unprivileged_values": sorted(unprivileged_values), "privileged_values": sorted(privileged_values), "validation_tprs": validation_tprs,
                "group_mapping": group_mapping,}
    
    fair_model = FairModel(name="Post: AIF360 Reject Option Classification", features=list(X_tr.columns), protected_cols=list(protected_cols), 
                           predictor=predictor, threshold=float(roc_model.classification_threshold), outcome_col=outcome_col, positive_label=1, metadata=metadata,)
   
   
    return evaluate_run("Post: AIF360 Reject Option Classification", y_te, p_test, yhat_test, A_te, fair_model=fair_model, test_index=X_te.index,
                         notes=("AIF360 RejectOptionClassification fitted on validation data: " f"threshold={roc_model.classification_threshold:.4f}, " f"margin={roc_model.ROC_margin:.4f}."),)
...
def run_input_repair(model_name, params, X_tr, X_va, X_te, y_tr, y_va, y_te, A_tr, A_va, A_te, protected_cols, all_df_train, outcome_col=None):
    ...
    input_repair_predictor = InputRepairPredictor(features=list(X_tr.columns), protected_cols=list(protected_cols), preprocessor=prep, 
                                                  estimator=clf, X_repair_reference=X_repair_reference, A_repair_reference=A_repair_reference, threshold=0.5,)
    

    fair_model = FairModel(name="Post: Input Repair (z-align)", features=list(X_tr.columns), 
                           protected_cols=list(protected_cols), predictor=input_repair_predictor, 
                           threshold=0.5, outcome_col=outcome_col, positive_label=1, 
                           metadata={"source": "FairSelect", "technique": "Post:Input Repair", 
                                     "model_name": model_name, "model_params": 
                                     params, "postprocessing": "input_repair_standardize_by_group", "repair_reference": "train_plus_validation", "threshold": 0.5,},)
    
    
    return evaluate_run("Post: Input Repair (z-align)", y_te.to_numpy(), p, yhat, A_te, fair_model=fair_model, test_index=X_te.index,)



def run_reject_option_kamiran(model_name, params, X_tr, X_va, X_te, y_tr, y_va, y_te, A_tr, A_va, A_te, protected_cols, 
                              all_df_train, fairness_objective="spd", theta_grid=None, base_threshold=0.5, fairness_bound=None, max_acc_drop=None, 
                              unprivileged_values=None, outcome_col=None,):
    """
    ...
    def aod(y_true, yhat, A):
        ...
        cr_u = confusion_rates(np.asarray(y_true)[unpriv], np.asarray(yhat)[unpriv])
        cr_p = confusion_rates(np.asarray(y_true)[priv], np.asarray(yhat)[priv])
        return float(0.5 * ((cr_u.get("TPR", np.nan) - cr_p.get("TPR", np.nan)) + (cr_u.get("FPR", np.nan) - cr_p.get("FPR", np.nan))))
    # -----------------------------
    ...
    for theta in theta_grid:
        ...
        candidates.append((float(theta), float(fair), float(val_acc), float(acc_drop), float(np.mean(in_critical)), bool(meets_acc), bool(meets_fair)))
    if len(candidates) == 0:
    ...
    print(f"Selected theta={best_theta:.3f} with fairness={best_fair:.4f}, " f"val_acc={best_acc:.4f}, crit_region={best_crit_frac:.4f}")
    # -----------------------------
    ...
    kamiran_predictor = KamiranRejectOptionPredictor(features=list(X_tr.columns), protected_cols=list(protected_cols), 
                                                     preprocessor=prep, estimator=clf, unprivileged_values=unprivileged_values, theta=best_theta,
                                                       base_threshold=base_threshold,)
    fair_model = FairModel(name=f"Post: Kamiran Reject Option (theta={best_theta:.3f}, obj={fairness_objective})", 
                           features=list(X_tr.columns), protected_cols=list(protected_cols), predictor=kamiran_predictor, 
                           threshold=base_threshold, outcome_col=outcome_col, positive_label=1, 
                           metadata={"source": "FairSelect", "technique": "Post:Reject-Option Kamiran", "model_name": model_name, "model_params": params, 
                                     "postprocessing": "kamiran_reject_option", "fairness_objective": fairness_objective, 
                                     "theta": float(best_theta), "base_threshold": float(base_threshold), "unprivileged_values": sorted(list(unprivileged_values)), 
                                     "validation_fairness": float(best_fair), "validation_accuracy": float(best_acc), "validation_accuracy_drop": float(best_acc_drop), 
                                     "validation_critical_region_fraction": float(best_crit_frac), "fairness_bound": fairness_bound, "max_acc_drop": max_acc_drop,},)
    
    
    return evaluate_run(f"Post: Kamiran Reject Option (theta={best_theta:.3f}, obj={fairness_objective})",
                         y_te, p_te, yhat_te, A_te, fair_model=fair_model, test_index=X_te.index,)