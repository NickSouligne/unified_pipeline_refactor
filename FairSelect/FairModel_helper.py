...
class AIF360RejectOptionPredictor:
    ...
    def __init__(
        ...
        self.group_mapping = {
            str(group): float(code) for group, code in group_mapping.items()
        }
    ...
    def _make_dataset(self, groups, probabilities):
        ...
        frame = pd.DataFrame({
            self.GROUP_COL: encoded,
            self.LABEL_COL: (probabilities >= 0.5).astype(float),
        })
        ...
        dataset = BinaryLabelDataset(
            ...
            protected_attribute_names=[self.GROUP_COL],
        )
    ...
    def predict(self, df):
        ...
        return np.asarray(
            self.roc_model.predict(dataset).labels, dtype=int
        ).ravel()
...
class CombinedPipelinePredictor:
    ...
    def __init__(
        ...
        *,
        ...
        multiaccuracy=False,
        reject_option_model=None,
        reject_option_group_mapping=None,
        X_val=None,
        y_val=None,
        A_val=None,
        p_val=None,
        ...
        self.reject_option_group_mapping = {
            str(group): float(code)
            for group, code in (reject_option_group_mapping or {}).items()
}
    ...
    def _apply_reject_option(self, df, probabilities):
        ...
        encoded_groups = (
            self.make_group(df)
            .astype(str)
            .map(self.reject_option_group_mapping)
            .fillna(-1.0)
            .astype(float)
            .to_numpy()
        )
        ...
        frame = pd.DataFrame({
            "__aif360_group__": encoded_groups,
            "__aif360_label__": (probabilities >= 0.5).astype(float),
        })
        ...
        dataset = BinaryLabelDataset(
            ...
            unfavorable_label=0.0,
            df=frame,
            label_names=["__aif360_label__"],
            protected_attribute_names=["__aif360_group__"],
        )
        dataset.scores = probabilities.reshape(-1, 1)

        return np.asarray(
            self.reject_option_model.predict(dataset).labels, dtype=int
        ).ravel()
